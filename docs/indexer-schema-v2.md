...
erDiagram
    ...
    Pool {
        ...
        Int binStepBps "optional, V2 only"
        Int baseFee "optional, V2 only"
    }
...
**Fields:**
- `id`: Unique asset ID of the NFT representing this position
- `pool`: Reference to the pool
...
- `totalLiquidityShares`: Total supply of liquidity shares for the bin
- `volumeX`: Volume of token X traded
- `volumeY`: Volume of token Y traded
- `feesX`: Fees accrued in token X
- `feesY`: Fees accrued in token Y
