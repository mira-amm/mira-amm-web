...
use crate::interface::{
    AddLiquidityScript, AddLiquidityScriptConfigurables, MiraAmmContract,
    MiraAmmContractConfigurables, PoolId, RemoveLiquidityScript,
    RemoveLiquidityScriptConfigurables, SwapExactInputScript, SwapExactInputScriptConfigurables,
    SwapExactOutputScript, SwapExactOutputScriptConfigurables, ADD_LIQUIDITY_SCRIPT_BINARY_PATH,
    AMM_CONTRACT_BINARY_PATH, REMOVE_LIQUIDITY_SCRIPT_BINARY_PATH,
...
use fuels::prelude::{
    AssetId, Bech32ContractId, Contract, LoadConfiguration, Result, ScriptTransaction, TxPolicies,
    VariableOutputPolicy, ViewOnlyAccount, WalletUnlocked,
};
...
impl MiraAmm {
    pub async fn deploy(
        ...
        let configurables = match lp_fees {
            Some((volatile_fee, stable_fee)) => MiraAmmContractConfigurables::default()
    ...
    pub async fn remove_liquidity(
        ...
        let (inputs, outputs) =
            get_transaction_inputs_outputs(&self.wallet, &vec![(lp_asset_id, liquidity)]).await;
        self.remove_liquidity_script
            .main(
                pool_id,
                liquidity,
                amount_0_min,
