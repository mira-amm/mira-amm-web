...
export class FetchService {
  ...
  private async errorEarly(
    ...
    switch (response.status) {
      case 404:
        return Result.error(
          new AppError("NotFoundError", `Resource not found: ${errorText}`, 404)
        );
      default:
        return Result.error(
          new AppError(
            "InternalError",
            `An error occurred while fetching data: ${errorText}`,
            500
          )
        );
    }
  ...
  private async validateResponse<T>(
    ...
    if (!result.success) {
      return Result.error(
        new AppError("BadGateway", "Bad data received from source", 502)
      );
    }
