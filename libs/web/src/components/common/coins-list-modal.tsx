...
export function CoinsListModal({
  ...
  return (
    <>
      <div className="flex gap-[10px] p-[14px_12px] rounded-lg text-[var(--content-grey)] bg-[var(--background-grey-dark)]">
        <SearchIcon />
        <input
          ref={inputRef}
          type="text"
          ...
          className="flex-1 bg-transparent border-none text-[var(--content-primary)] text-base outline-none"
        />
      </div>
        ...
        <div>
          ...
          {allCoins.map((asset) => (
            <div
              ...
              className="p-2 px-4 rounded-lg hover:bg-[var(--background-grey-dark)] cursor-pointer"
            >
...
function EmptySearchResults({ value }: { value: string }) {
  return (
    <div className="px-4 py-2 text-[var(--content-dimmed-light)]">
      {value && (
        <span>
          No results found for{" "}
          <span className="text-[var(--content-primary)]">"{value}"</span>
        </span>
      )}
    </div>
  );
