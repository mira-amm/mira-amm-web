...
export function useSwapRouter(
  ...
  useEffect(() => {
    if (
      ...
      console.log("🔄 Preloading pools for route change:", {
      ...
      amm
        .preloadPoolsForRoutes(poolPaths, ammCacheOptions)
        .then(() => {
          console.log("✅ Successfully preloaded pools for routes");
        })
        .catch((error: any) => {
          console.warn("❌ Failed to preload pools for routes:", error);
        });
