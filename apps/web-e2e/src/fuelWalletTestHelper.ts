import { FUEL_MNEMONIC, FUEL_WALLET_PASSWORD, getButtonByText, getByAriaLabel, expectButtonToBeEnabled, hasText } from '@fuels/playwright-utils';
...
export class FuelWalletTestHelper {
  ...
  static async walletSetup({
    ...
    const createNewWalletButton = signupPage
      .getByRole('heading', { name: 'Create new wallet' })
    ...
    await signupPage.locator('button').getByText('Next: Confirm Phrase').click();
    ...
    const accountsButton = getByAriaLabel(fuelWalletTestHelper.walletPage, 'Accounts');
    await accountsButton.click();
    const addAccountButton = getByAriaLabel(fuelWalletTestHelper.walletPage, 'Import from private key');
    await addAccountButton.click();
    const importAccountInput = getByAriaLabel(fuelWalletTestHelper.walletPage, 'Private Key');
    await importAccountInput.fill(process.env.FUEL_WALLET_PRIVATE_KEY);
    const accountNameInput = getByAriaLabel(fuelWalletTestHelper.walletPage, 'Account Name');
    await accountNameInput.fill("Derek the Dino");
    const importButton = getButtonByText(fuelWalletTestHelper.walletPage, 'Import');
