...
const NATIVE_BRIDGE_MINTER_CONTRACT = '0x4ea6ccef1215d9479f1024dff70fc055ca538215d2c8c348beddffd54583d0e8';

const providerPromise = Provider.create(NetworkUrl);

export const useAssetPrice = (assetId: string | null): { price: number | null, isLoading: boolean } => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const src7Contract = new Contract(NATIVE_BRIDGE_MINTER_CONTRACT, src7Abi, provider!);
      ...
      if (assetId === ETH_ASSET_ID) {
        ...
        const result = await src7Contract.functions
          ...
          .addContracts([
            ...
            new Contract('0x0ceafc5ef55c66912e855917782a3804dc489fb9e27edfd3621ea47d2a281156', src7Abi, provider!),
          ])
          .get();
    ...
    enabled: !!assetId && (contractId === NATIVE_BRIDGE_MINTER_CONTRACT || assetId === ETH_ASSET_ID),
    staleTime: Infinity,
  });
